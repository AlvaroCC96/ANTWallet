  ...
  return (
    <div className="min-h-screen bg-deep flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-card border border-deep-darker/60 rounded-2xl p-6 space-y-4 text-center">
        ...
        <h2 className="text-xl font-bold text-ink">Acceso pendiente</h2>
        <p className="text-sm text-muted">
          Tu cuenta <span className="text-ink">{user?.email}</span> aún no tiene acceso a ANTWallet. Pídele al
          administrador que te active desde el panel de Admin.
        </p>
        <button
          onClick={logOut}
          className="w-full bg-card-alt hover:bg-deep-darker text-ink text-sm font-medium py-2 rounded-lg transition-colors flex items-center justify-center gap-1.5"
        >
