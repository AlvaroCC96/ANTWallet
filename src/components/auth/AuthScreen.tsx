...
export function AuthScreen() {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      setError(getAuthErrorMessage(extractAuthErrorCode(err)))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-deep flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-card border border-deep-darker/60 rounded-2xl p-6 space-y-5 text-center">
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-white hover:bg-gray-100 disabled:opacity-50 text-gray-800 font-semibold py-2.5 rounded-lg transition-colors flex items-center justify-center gap-2"
        >
